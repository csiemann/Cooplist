...
export async function initDatabase(): Promise<Database> {
  ...
  // Adicionar colunas 'max_uses' e 'uses' para convites reutilizáveis
  await db.exec('ALTER TABLE invites ADD COLUMN max_uses INTEGER').catch(() => {});
  await db.exec('ALTER TABLE invites ADD COLUMN uses INTEGER DEFAULT 0').catch(() => {});
